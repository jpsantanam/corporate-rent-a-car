document.addEventListener('DOMContentLoaded', function () {
    ...
    document.body.addEventListener('submit', function (event) {
        ...
        if (event.target.id === 'manage-rent') postPutRents(event); // essa linha é o motivo pra `name="_id"` no form
            ...
            if (searchParam.includes('status=ativo') || searchParam.includes('status=inativo')) searchRents(`/rents${searchParam}`);
            else {
    ...
    function prepareQuery(event) {
        ...
        if (searchParam.includeActive) queryParams.push('status=ativo');
        if (searchParam.includeInactive) queryParams.push('status=inativo');
