...
const fillMaintenancesTable = async (maintenances) => {
    ...
    maintenances.forEach(maintenance => {
        ...
        if (maintenance.type === 'corretiva') tr.classList.add('table-active');
...
function prepareQuery(event) {
    ...
    if (searchParam.includePreventive) queryParams.push('type=preventiva');
    if (searchParam.includeCorrective) queryParams.push('type=corretiva');
...
const searchMaintenance = async (event) => {
    ...
    if (searchParam.includes('type=preventiva') || searchParam.includes('type=corretiva')) {
        const maintenances = await getMaintenancesByParam(searchParam);
        fillMaintenancesTable(maintenances);

    } else {
        document.getElementById('maintenances-query-results').setAttribute('hidden', true);
        showToast('Selecione ao menos um tipo de manutenção', "warning");
    }
}
...
function createMaintenance(data) {
    fetch('https://corporate-rent-a-car-app.onrender.com/maintenances', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'userCookie': getCookie('user') || ''
...
function updateMaintenance(id, data) {
    fetch(`https://corporate-rent-a-car-app.onrender.com/maintenances/${id}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            'userCookie': getCookie('user') || ''
