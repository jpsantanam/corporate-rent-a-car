...
function createLog(req, res, next, action) {
    req.body = {
        action: action,
        entity: "o veículo",
        target: `${req?.body?.brand} ${req?.body?.model} ${req?.body?.motorization} ${req?.body?.year}`
    }
...
const getByQuery = async (req, res, next) => {
    try {
        ...
        const filteredVehicles = allVehicles.filter(vehicle => {
            return JSON.stringify(vehicle).toLowerCase().includes(searchParam);
        })
...
const filterVehicle = async (req, res, next) => {
    ...
    try {
        ...
        if (Array.isArray(motorization)) {
            filters.motorization = { [Op.or]: motorization.map(motor => ({ [Op.like]: `%${motor}%` })) };
