...
const getByQuery = async (req, res, next) => {
    try {
        ...
        const allCompanies = await companies.findAll({ raw: true, nest: true, include: [addresses, representatives, partner] });
        ...
        if (filteredCompanies.length === 0) {
            return res.status(404).send(`Nenhum cliente encontrado com o parâmetro ${searchParam}`);
        }
